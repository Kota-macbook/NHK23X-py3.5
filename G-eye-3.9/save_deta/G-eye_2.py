...
h, w = img.shape[:2]
# ポールの色を抽出
img_colorfiltered = def_colorfilter.colorfilter(
    img, (23, 189, 212), (4, 61, 108))
...
for i in range(1, nLabels):
    ...
    if xc-60 >= 0 and yc+60 <= h:
        cv2.putText(img_num, str(i), (xc-60, yc+60),
                    font, scale, color, 4, cv2.LINE_AA)
    else:
        cv2.putText(img_num, str(i), (xc, yc), font,
                    scale, color, 4, cv2.LINE_AA)
